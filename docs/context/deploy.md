...
Framework: standard `#[test]` + `tokio-test` (dev-dep) for async. **4 unit tests** in `src/config.rs` (user ID parsing, tool verbosity). No integration tests or `tests/` directory.

## CI

**None configured.** No `.github/workflows/`, no Dockerfile. Minimal GitHub Actions starter:
...
Resolution: env var → `./.env` → `~/.config/telepi/.env`.
...
|----------|---------|-------------|
| `TELEPI_WORKSPACE` | — | Projects directory for Pi agent |
...
Generates `~/Library/LaunchAgents/com.telepi.plist` (`RunAtLoad`, `KeepAlive` on failure). Logs at `~/Library/Logs/TelePi/`.
...
Generates `~/.config/systemd/user/telepi.service` (`Restart=on-failure`, `RestartSec=5`). Logs at `~/.local/state/telepi/logs/`.
...
```bash
telepi status    # Version, config, service status, extension status
```
...
cargo build --release
cargo test
cp .env.example .env                    # Fill in TELEGRAM_BOT_TOKEN + TELEGRAM_ALLOWED_USER_IDS
